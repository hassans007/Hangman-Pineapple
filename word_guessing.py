...
# Word_bank.py with categories defined in it
...
class WordGuessingGame(tk.Tk):
    ...
    def show_start_menu(self):
        ...
        start_button = tk.Button(self.current_frame, text="Start Game", font=BUTTON_FONT,
                                 command=self.start_game)  # Create a button widget
        start_button.pack()  # Pack the button into the frame
        quit_button = tk.Button(self.current_frame, text="Quit Game", font=BUTTON_FONT,
                                command=self.quit_game)  # Create a button widget
    ...
    def choose_word(self, category):
        ...
        alphabet_frame = tk.Frame(self.current_frame,
                                  background=BACKGROUND_COLOR_SECONDARY)  # Create a new frame for alphabet buttons
    ...
    def handle_alphabet_button(self, char, word_label, tries_label, incorrect_guess_label, congrats_label):
        ...
        if char not in self.word:  # Check if the guessed letter is not in the word
            ...
            incorrect_guess_label.config(
                text=f"Incorrect guesses: {' '.join(self.guessed_letters)}")  # Update incorrect guesses label
            if self.tries == 0:  # Check if no tries left
                word_label.config(
                    text=f"Sorry, you're out of tries! The word was: {self.word}")  # Update the word label
            ...
            new_display = "".join(
                [letter if letter in self.guessed_letters else "_" for letter in self.word])  # Update the display
